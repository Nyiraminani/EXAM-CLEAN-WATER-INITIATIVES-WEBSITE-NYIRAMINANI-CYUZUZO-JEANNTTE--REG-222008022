...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!empty($user_name) && !empty($password) && !empty($email) && !empty($gender) && !empty($age) && !empty($telephone) && !empty($country) && !is_numeric($user_name)) {
        ...
        $stmt = $con->prepare("INSERT INTO jeannette (user_id, user_name, password, email, gender, age, telephone, country) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssss", $user_id, $user_name, $hashed_password, $email, $gender, $age, $telephone, $country);
        $stmt->execute();
...
<head>
    ...
    <style type="text/css">
        ...
        #box {
            ...
            background-image: url(wp2758182.gif);
            margin: auto;
            width: 300px;
            padding: 20px;
        }
...
<div id="box">
    <form method="POST">
        ...
        <input id="text" type="tel" name="telephone"><br><br>
